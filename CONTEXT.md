...
**Run**:
Console 里一次操作：可含多个 Target，固定顺序 android → ios → harmony（跳过未勾选），同一日志流（带平台前缀），一个取消打断整批。同一时刻最多一个会改 App Root 的构建类 Run；上传类可与构建并行。上传成功后：有 Merged Install Page 则优先打开；否则同窗多卡展示各平台二维码，并显示本次 Install Note（若有）。
